...
export default function ProductCard({ product }) {
  ...
  const rating = ratingCount > 0 ? (product.farmer?.farmerProfile?.rating || 0) : 0
  ...
  return (
    <Link href={`/buyer/products/${product.id}`}
      ...
      <div className="h-24 bg-green-50 flex items-center justify-center text-5xl overflow-hidden">
        {mainImage ? (
      ...
      <div className="p-3">
        ...
        <div className="flex items-center gap-1 mt-0.5">
          <div className="text-xs flex">{renderStars(rating)}</div>
          <p className="text-xs text-gray-500">{rating.toFixed(1)}</p>
        </div>
